...
export class AudioService {
  ...
  private getAppropriateFormat(info: videoInfo): videoFormat {
    const formats: videoFormat[] = ytdl.filterFormats(
      info.formats,
      'audioonly'
    );
  ...
  private extractAudio(data: IExtractAudioData): void {
    ...
    if (ext == 'mp3') {
      ...
      const process: ChildProcess = exec(
        `ffmpeg -i ${filename}.mp4 -acodec ${codec} -vn -ab ${bitrate}k -ar 44100 -ac 2 ${filename}.${ext}`,
        callback
      );
    ...
    if (ext == 'wav') {
      codec = 'pcm_s16le';
      const process: ChildProcess = exec(
        `ffmpeg -i ${filename}.mp4 -acodec ${codec} -vn -ar 44100 -ac 2 ${filename}.${ext}`,
        callback
      );
      process.on('exit', (exitCode: number) => {
        unlink(filename + '.mp4', () => {
